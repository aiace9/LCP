...
contains
  ...
  subroutine initial(nequil,cum)
    ...
    print*, 'starting conditions: rnd, up, down, chess_board(chb), up_down(ud), down_up(du) '
    read *, start
    select case (start)
      case ('rnd')
        do y = 1,L
...
! Choosing the interaction parameter  J=1, 
! possible energy variations per spin flip are -8,-6,-4,-2,0,+4,+8: 
    do dE = -8,8,2
       w(dE) = exp(-dE/T)
    end do
    accept = 0
...
! also here the factor 2 is to account for the variation
  ...
  subroutine output(cum)
    ...
    print *, "c =", (e2ave - eave**2)/(1 * T *N)
    print *, "chi =", (m2ave - mave**2)/(1 * T *N)
...
program ising
  ...
  open(unit=1, file='step_E_M.dat', iostat=ios, status="unknown", action="write")
  if ( ios /= 0 ) stop "Error opening file step_E_M"
