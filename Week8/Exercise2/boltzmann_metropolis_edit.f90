...
contains
  ...
  subroutine initial(nequil,vcum,ecum,e2cum)
    ...
    write(unit=9,fmt=*)"# <E0>    :",E/20
    write(unit=9,fmt=*)"# <v0>    :",sqrt(dot_product(vel,vel)/20)
  ...
  subroutine Metropolis() 
    ...
    do i = 1, 20, 1
      ...
      vtrial = vel(i) + dv                     ! trial velocity v
      de = 0.5 * (vtrial*vtrial - vel(i)*vel(i))  ! corresponding variation of E
      call random_number(rnd)
      if (de >= 0.0) then
         if ( exp(-beta*de) < rnd ) cycle ! trial step not accepted
      end if
  ...
  subroutine data(vcum,ecum,e2cum)
    ...
    Ecum = Ecum + E 
    E2cum = E2cum +  E * E
    vcum = vcum + sum(vel, dim=1, mask=(vel>0))
    call probability() 
  end subroutine data
  ...
  subroutine averages(nequil,vcum,Ecum,E2cum) 
    ...
    integer :: ibin
    znorm  = 1.0/(nmcs*20)
    accept = accept / ((nmcs+nequil)*20) ! acceptance ratio
